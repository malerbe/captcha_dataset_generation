...
class ImageCaptcha:
    ...
    @staticmethod
    def create_noise_curve(image: Image, color: ColorTuple, mask: Image = None) -> Image:
        w, h = image.size
        ...
        if mask:
            Draw(mask).arc(points, start, end, fill=color)
        return image, mask
    ...
    def _draw_character(
        ...
        #m = createImage('RGBA', (int(w) + dx1, int(h) + dy1))
        Draw(im).text((dx1, dy1), c, font=font, fill=color)
        #Draw(m).text((dx1, dy1), c, font=font, fill=color)
        ...
        im = im.rotate(
            self.character_rotate[0] + (randbits / (2**32)) * (self.character_rotate[1] - self.character_rotate[0]),
            Resampling.BILINEAR,
            expand=True,
        )
        ...
        # m = m.resize((w2, h2))
        # m = m.transform((int(w), int(h)), Transform.QUAD, data)
        return im #, m

    def make_mask(self, char_img):
        # Tout pixel ayant au moins un canal > 0 est une lettre
        gray = ImageOps.grayscale(char_img)
        mask = gray.point(lambda x: 255 if x > 0 else 0)
        return mask
    ...
    def create_captcha_image(
        ...
        if segmentation:
            mask = createImage('RGB', (self._width, self._height), 0)
            mask_draw = Draw(mask) 
        ...
        for c in chars:
            if secrets.randbits(32) / (2**32) > self.word_space_probability:
                char_img = self._draw_character(" ", draw, color)
                images.append(char_img)
                if segmentation:
                    masks.append(self.make_mask(char_img))
            char_img = self._draw_character(c, draw, color)
        ...
        for k in range(len(images)):
            w, h = images[k].size
            img_mask = images[k].convert('L').point(self.lookup_table)
            m_mask = masks[k].convert('L').point(self.lookup_table)
            image.paste(images[k], (offset, int((self._height - h) / 2)), img_mask)
            mask.paste(masks[k], (offset, int((self._height - h) / 2)), m_mask)
    ...
    def generate_image(self, chars: str,
        ...
        im, mask = self.create_captcha_image(chars, color, background, segmentation=segmentation)
    ...
    def generate(self, chars: str, format: str = 'png',
        ...
        :param fg_color: foreground color of the text in rgba format (r,g,b,a).
        """
        im, mask = self.generate_image(chars, bg_color=bg_color, fg_color=fg_color)
        out = BytesIO()
    ...
    def write(self, chars: str, output: str, format: str = 'png',
        ...
        :param fg_color: foreground color of the text in rgba format (r,g,b,a).
        """
        im, mask = self.generate_image(chars, bg_color=bg_color, fg_color=fg_color, segmentation=segmentation)
        im.save(output, format=format)
        if mask:
            mask.save(output_mask, format=format)
